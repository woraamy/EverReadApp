...
struct ActivityCard: View {
    ...
    var actionText: String {
        switch action {
        case "write a review":
            return "Write a review for \(book_name)"
        case "add want to read":
            return "Add \(book_name) to want to read"
        case "add currently reading":
            return "Currently reading \(book_name)"
        case "add finished":
            return "Finish reading \(book_name)"
        default:
            return "do something"
        }
    }
...
#Preview {
    ActivityCard(name: "John Reader", action: "write a review", recentDay: 2, book_name: "", profile: "")
}
